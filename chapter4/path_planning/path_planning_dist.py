...
class PathPlanning:
    ...
    def make_grid_world(self):
        ...
        elif self.world_no == 5:
            self.width = 5       # width of the grid world
            self.height = 5      # height of the grid world    
            self.start = (0, 0)  # start point
            self.goal = (4, 4)   # goal point
            self.DIAGRAM_WALLS = []
            self.DIAGRAM_WALLS = [(0,4), (0,2), (0,3),(1,2),(1,3), (1,4), (2,4), (3,1),(3,2), (3,3), (3,4)]
            #self.DIAGRAM_WALLS = [(1,2), (3,1), (3,3)]
            print('DIAGRAM_WALL5=', self.DIAGRAM_WALLS)   
...
class DrawMap():
    ...
    def paint_tile(self,path):
        ...
        self.cmap.set_bad('cyan')  # cyan # bad_valueの格子は cyan path demu for book white
        
    def show(self, path, closed_cells):
        visit_order = dict()
        ...
        for cell in closed_cells.keys():
            ...
            print(f'cell{cell}')
            self.map[cell] = order
            if order >= 0 and cell != self.start and cell != self.goal: # for book demu
                self.ax.text(cell[0],cell[1], str(order), va='center', ha='center',
...
def breadth_first_search(graph, start, goal, title):
    ...
    while not open_cells.empty():
        #print(f'step{step}:open')
        #with open_cells.mutex:
        #    print(open_cells.queue)
